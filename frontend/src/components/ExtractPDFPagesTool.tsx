...
import { Upload, FileText, FileSearch, ArrowRight, RefreshCw, AlertCircle, X, Check, Package, FileDown } from "lucide-react";
...
export function ExtractPDFPagesTool() {
    ...
    /* ── Extract ── */
    const handleExtract = useCallback(async (mode: "single" | "separate") => {
        ...
        try {
            ...
            params.set("extractMode", mode);

            const endpoint = `/api/edit/extract-pages?${params.toString()}`;
            ...
            <section className="hero-gradient py-16 sm:py-20 relative overflow-hidden">
                <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 text-center relative z-10">
                    ...
                    <p className="text-lg text-gray-400 max-w-2xl mx-auto">
                        Select specific pages from your PDF and extract them into a new document or separate files.
                    </p>
                </div>
            </section>
                    ...
                    {status?.status === "COMPLETED" ? (
                        <div className="text-center">
                            ...
                            <button onClick={handleReset}
                                ...
                                <RefreshCw className="h-4 w-4" /> Extract from another file
                            </button>
                        </div>
                    ) : !file ? (
                        <>
                        ...
                        <>
                            ...
                            <div className="flex flex-wrap gap-2 mb-4">
                                <button onClick={selectAll}
                                    className="px-3 py-1.5 text-xs rounded-md border border-white/10 bg-white/[0.03] text-gray-300 hover:border-emerald-500/30 hover:text-white transition-all">
                                    Select All
                                </button>
                            ...
                            <div className="flex gap-2 mb-6">
                                <input
                                    ...
                                    className="flex-1 bg-white/[0.03] border border-white/10 rounded-lg px-4 py-2 text-sm text-white placeholder-gray-600 focus:border-emerald-500 focus:outline-none transition-colors"
                                />
                                <button onClick={applyRangeInput}
                                    className="px-4 py-2 text-xs rounded-lg border border-white/10 bg-white/[0.03] text-gray-300 hover:border-emerald-500/30 hover:text-white transition-all">
                                    Apply Range
                                </button>
                            </div>
                            ...
                            {loading ? (
                                <div className="text-center py-12">
                                    <div className="inline-block h-8 w-8 border-2 border-emerald-500/30 border-t-emerald-400 rounded-full animate-spin" />
                                            ...
                                            {p.thumbnail ? (
                                                <img src={p.thumbnail} alt={`Page ${p.index}`}
                                                    className={`w-full aspect-[3/4] object-cover transition-all
                                                        ${p.selected ? "ring-2 ring-emerald-500/30 ring-inset" : ""}`} />
                                            ) : (
                            ...
                            {selectedCount > 0 && !isProcessing && (
                                <div className="space-y-3">
                                    <p className="text-sm text-gray-400 text-center">
                                        <span className="text-emerald-400 font-semibold">{selectedCount}</span> page{selectedCount > 1 ? "s" : ""} selected for extraction
                                    </p>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        <button onClick={() => handleExtract("single")}
                                            className="flex items-center justify-center gap-2 py-3.5 rounded-lg border-2 border-emerald-500/30 bg-emerald-500/5
                                                text-emerald-400 text-sm font-semibold hover:bg-emerald-500/10 hover:border-emerald-500/50 transition-all">
                                            <FileDown className="h-4 w-4" /> Extract as Single PDF
                                        </button>
                                        <button onClick={() => handleExtract("separate")}
                                            className="flex items-center justify-center gap-2 py-3.5 rounded-lg border-2 border-teal-500/30 bg-teal-500/5
                                                text-teal-400 text-sm font-semibold hover:bg-teal-500/10 hover:border-teal-500/50 transition-all">
                                            <Package className="h-4 w-4" /> Extract as Separate PDFs (ZIP)
                                        </button>
                                    </div>
                                </div>
                            )}
